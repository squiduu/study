...
from decomposition import (SEED_VISUALIZATION, get_max_batch_size,
                           get_or_compute, get_random_dirs)
from estimators import get_estimator
from models import get_instrumented_model
from netdissect import easydict, nethook, proggan, zdataset
from netdissect.modelconfig import create_instrumented_model
from utils import pad_frames


# Find index and corresponding value of the closest vector with X in p vectors
def x_closest(p):
...
# Make a .gif file with input images to show the results
...
# Make a .mp4 file with input images to show the results
def make_mp4(imgs, duration_secs, outname):
    ...
    command = [
        ...
        "rawvideo",
        "-s",
        f"{resolution[0]}x{resolution[1]}",  # size of one frame
        "-pix_fmt",
        "rgb24",
        "-r",
        f"{fps}",
    ...
    frame_data = np.concatenate([(x * 255).astype(np.uint8).reshape(-1)
                                 for x in imgs])
...
# Make a grid to show many images at once
# Go to notebook_utils.py
def make_grid(
    latent,
    lat_mean,
    lat_comp,
...
if __name__ == "__main__":
    ...
    args = Config().from_args()  # call all the arguements that users set
    ...
    has_gpu = torch.cuda.is_available()  # check using GPU or not
    device = torch.device("cuda" if has_gpu else "cpu")
    layer_key = args.layer  # layer to analyze
    layer_name = layer_key  # layer_key.lower().split('.')[-1]

    # convert from relative path to absolute path
    basedir = Path(__file__).parent.resolve()
    outdir = basedir / "out"  # set output directory

    # Load model
    #
    # Instrumented model is defined in nethook
    # Go to nethook.py
    inst = get_instrumented_model(args.model,
                                  args.output_class,
                                  layer_key,
                                  device,
                                  use_w=args.use_w)
    # Network model to analyze
    model = inst.model
    ...
    if len(feature_shape) != 4:  # non-spatial
        ...
        # Only batch fixed => whole activation volume used
    ...
    # Load or compute components using PCA
    dump_name = get_or_compute(args, inst)

    # Do not contain object arrays
    ...
    # Percentage of zero values in components
    ...
    for edit_mode in edit_modes:
        ...
        plt.suptitle(
            f"{args.estimator.upper()}: {model.name} - {layer_name}, {get_edit_name(edit_mode)} edit",
            size=16,
        )
        make_grid(
        ...
        plt.savefig(outdir_summ / f"components_{get_edit_name(edit_mode)}.jpg",
                    dpi=300)
        show()

    # Choose to make .mp4 file to show the results
    if args.make_video:
        components = 15
        instances = 150
            ...
            for c in range(components):
                for edit_mode in edit_modes:
                    frames = make_grid(
                        tensors.Z_global_mean,
                        tensors.Z_global_mean,
                        tensors.Z_comp[c:c + 1, :, :],
                        tensors.Z_stdev[c:c + 1],
                        tensors.X_global_mean,
                        tensors.X_comp[c:c + 1, :, :],
                        tensors.X_stdev[c:c + 1],
                        n_rows=1,
                        n_cols=instances,
                        scale=sigma,
                        make_plots=False,
    ...
    for edit_mode in edit_modes:
        ...
        plt.savefig(outdir_summ /
                    f"random_dirs_{get_edit_name(edit_mode)}.jpg",
                    dpi=300)
        show()
    ...
    for img_idx in trange(n_random_imgs, desc="Random images", ascii=True):
        ...
        for edit_mode in edit_modes:
            ...
            plt.savefig(
                outdir_summ /
                f"samp{img_idx}_real_{get_edit_name(edit_mode)}.jpg",
                dpi=300,
            )
            show()
            ...
            for sigma in [args.sigma, 3 * args.sigma]:  # [2, 5]:
                for edit_mode in edit_modes:
                    ...
                    for c in range(components):
                        frames = [
                            x
                            for _, x in imgs[c * instances:(c + 1) * instances]
                        ]
                        frames = frames + frames[::-1]
                        make_mp4(
                            frames,
